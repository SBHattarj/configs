...
${PR_CYAN}%(?..${PR_RED})${PR_ULCORNER}${PR_HBAR}${PR_BLUE}(\
${PR_GREEN}%${PR_PWDLEN}<...<%~%<<\
${PR_BLUE})$(ruby_prompt_info)${PR_CYAN}%(?..${PR_RED})${PR_HBAR}${PR_HBAR}${(e)PR_FILLBAR}${PR_HBAR}${PR_BLUE}(\
${PR_CYAN}%(!.%SROOT%s.%n)${PR_YELLOW}@${PR_GREEN}%m:%l\
${PR_BLUE})${PR_CYAN}%(?..${PR_RED})${PR_HBAR}${PR_URCORNER}\

${PR_CYAN}%(?..${PR_RED})${PR_LLCORNER}${PR_BLUE}${PR_HBAR}(\
${PR_YELLOW}%D{%H:%M:%S}\
${PR_LIGHT_BLUE}%{$reset_color%}$(git_prompt_info)$(git_prompt_status)${PR_BLUE})${PR_CYAN}%(?..${PR_RED})${PR_HBAR}\
${PR_HBAR}\
>${PR_NO_COLOUR} '

# display exitcode on the right when > 0
return_code="%(?..%{$fg[red]%}%? ↵ %{$reset_color%})"
RPROMPT=' $return_code${PR_CYAN}%(?..${PR_RED})${PR_HBAR}${PR_BLUE}${PR_HBAR}\
(${PR_YELLOW}%D{%a,%b%d}${PR_BLUE})${PR_HBAR}${PR_CYAN}%(?..${PR_RED})${PR_LRCORNER}${PR_NO_COLOUR}'

PS2='${PR_CYAN}$%(?..${PR_RED}){PR_HBAR}\
${PR_BLUE}${PR_HBAR}(\
${PR_LIGHT_GREEN}%_${PR_BLUE})${PR_HBAR}\
${PR_CYAN}$%(?..${PR_RED}){PR_HBAR}${PR_NO_COLOUR} '
